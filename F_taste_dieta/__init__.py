...
from F_taste_dieta.db import set_DB_CONFIG,create_db

from F_taste_dieta.ma import ma

from F_taste_dieta.namespaces import nutrizionista_ns,paziente_ns
from F_taste_dieta.controllers.alimento_controller import AlimentoController
from F_taste_dieta.controllers.dieta_controller import DietaController,DietaPazienteController


from F_taste_dieta.utils.jwt_custom_decorators import NoAuthorizationException
...
def create_app():
    ...
    app.config.from_object('F_taste_dieta.config.DevelopmentConfig')
 ...
 #   @api.errorhandler(NoResultFound)
  #  def handle_NoResultFound_error(error: NoResultFound):
   ...
   # @api.errorhandler(NoAuthorizationException)
    ...
    nutrizionista_ns.add_resource(DietaController, '/dieta')
    nutrizionista_ns.add_resource(AlimentoController, '/alimenti')
    
    paziente_ns.add_resource(DietaPazienteController, '/dieta')
...
#    app.add_url_rule("/password_reset", view_func=TemplatePasswordChangerController.reindirizza, methods=['GET'])
 #   app.add_url_rule("/success", view_func=TemplatePasswordChangerController.successo, methods=['GET'])
  #  app.add_url_rule("/failure", view_func=TemplatePasswordChangerController.fallimento, methods=['GET'])
    ...
    def health_check():
        return {'message': 'API dieta è online'}, 200
